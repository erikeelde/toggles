...
internal class TogglesResolverTest {
    ...
    @Test
    fun `resolve invokes onFirstCreate when auto-creating configuration`() {
        val resolver = createResolver(addDefaultAutomatically = true)
        val state = ToggleState(null, emptyList(), listOf(defaultScope))
        // Note: insertConfiguration returns null because no real provider is registered,
        // so onFirstCreate will not be invoked. This tests the null-return path.
        var firstCreateCalled = false

        resolver.resolve(state, "key", Toggle.TYPE.STRING, "default") {
            firstCreateCalled = true
        }

        // insertConfiguration returns null (no provider), so onFirstCreate is not called
        assertEquals(false, firstCreateCalled)
    }
    ...
    fun `resolve returns default scope value when no selected scope override`() {
        ...
        // development scope has lower timestamp, so default scope is "selected"
        val state = ToggleState(
            config,
            listOf(defaultValue),
    ...
    fun `resolve auto-creates default value when configuration exists but default value missing`() {
        // When addDefaultAutomatically=true and configuration exists but no default-scope value,
        // the resolver tries to insert a configuration value. With no real provider,
        // the insert is a no-op but the logic path is exercised.
        val resolver = createResolver(addDefaultAutomatically = true)
        val config = buildConfiguration(id = 10L, key = "key")
        // No configuration values at all
        val state = ToggleState(
            config,
            emptyList(),
            listOf(defaultScope)
